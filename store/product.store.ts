import { TypeProductosResponse } from '@/src/types/productos'
...
export const productStore = create<Props>()((set, get) => ({
    ...
    obtenerProducto: (producto) => {
        ...
        const productfiltered = get().productos.filter(e=>e.nombre.toLowerCase().includes(search.toLowerCase()))
    set((state)=>({
    ...
    productosAgotados: () => {
        const result = get().productos?.reduce((acc, obj: Producto) => {
            if (obj.stock == 0) {
    ...
    dahsboardFunctionAgotados: () => {
        set(state => ({
            agotados: state.productos.filter((e: Producto) => e.stock < e.stock_min && e.stock !== 0)
        }))
